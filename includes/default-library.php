...
/*	md2html($text)
	...
	function md2html($text) {
		...
		//	Lists
			//	doesn’t (yet) handle indented text
...
//			}, $text);
			...
			$regexList = '/\n[*-]\s+?([\s\S]+?)(?=\n(?=-|\n\S))/m';
			$text = preg_replace_callback($regexList, function($data) {
				[, $text] = $data;
				...
				if(count($data) < 2) return sprintf("\n<ul>\n\t<li>%s</li>\n</ul>", trim($data[0]));
				else return sprintf("\n<ul>\n\t<li><p>\n%s\n</p></li>\n</ul>", implode("\n</p>\n<p>",$data));
			}, $text);
		...
		//	Code Blocks
			$regexCode = '/(\r?\n)(\t*)```(.*?)\r?\n([\s\S]*?)\r?\n\t?```/m';
			$text = preg_replace_callback($regexCode, function($data) {
				[, $br, $indent, $language, $text] = $data;

				$code = sprintf(
					'%s<pre><code%s>%s</code></pre>%s',
					$br,
					$language ?  sprintf(' class="language-%s"', $language) : '',
		...
		//	Inline Code: Eliminate Code Blocks First
